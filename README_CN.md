# 梦月音乐播放器

---

<p align="center">
    <img src="https://github.com/TypeDreamMoon/TypeDreamMoon_Assets/blob/main/Images/logo/MusicPlayerBanner.png" />
</p>

### [ENGLISH](README.md)

开发者 : [DreamMoon](https://github.com/TypeDreamMoon)

示例项目 : [链接](https://github.com/TypeDreamMoon/DreamMusicPlayer/tree/5.6-MusicPlayerSample)

支持引擎版本 : `5.0`,`5.1`,`5.3`,`5.4`,`5.5`,`5.6`, `5.7`

## 开发引擎 :

| 插件版本  | 引擎版本 |
|-------|------|
| 1.1.0 | 5.0  |
| 1.1.1 | 5.5  |
| 2.0.0 | 5.6  |
| 3.0.0 | 5.7  |

---

## 功能 :

1. 滚动歌词

2. 逐字歌词

3. 歌词文件运行时解析 [支持列表](https://github.com/typedreammoon/libdlp)

4. 音乐渐进渐出

5. 音乐歌单

6. 音乐封面 KMeans 算法主题色计算

7. 音频可视化

8. 播放器拓展

9. 点击歌词播放

10. 歌词文件导入

11. 歌曲导入

12. [歌曲Tag识别](https://github.com/taglib/taglib)

13. 歌词编辑器

14. 歌曲编辑器

15. 模块化

16. 音频拓展

17. [FLAC 解析](https://github.com/xiph/flac)

18. NCM 解析 (禁止商业用途 仅供学习使用)

19. Taskbar Thumbnail Toolbar

20. Windows SMTC支持

## 路线图 :

1. BPM识别

2. 网络播放

3. 音频渐入渐出

4. 智能识别切歌时间

5. QMC, MFLAC, MGG 解析 (禁止商业用途 仅供学习使用)

6. KGM 解析 (禁止商业用途 仅供学习使用)

7. KWM 解析 (禁止商业用途 仅供学习使用)

8. XM 解析 (禁止商业用途 仅供学习使用)

---

## 模块

| 模块名称                    | 模块描述         |
|-------------------------|--------------|
| DreamMusicPlayer        | 音乐播放器核心实现    |
| DreamMusicPlayerEditor  | 音乐播放器编辑器实现   |
| DreamMusicPlayerLyric   | 歌词文件解析实现     |
| DreamMusicPlayerNetwork | 音乐网络实现       |
| DreamMusicPlayerUMG     | 音乐播放器 UMG 实现 |

---

## 第三方工具 :

1. [音乐标签 pc 版 - vinlxc - 博客园](https://www.cnblogs.com/vinlxc/p/11347744.html)
2. [Releases · chenmozhijin/LDDC · GitHub](https://github.com/chenmozhijin/LDDC/releases)

---

## 帮助 :

- QQ群 : 466585194

- Email : 1067823908@qq.com

- Bilibili : [TypeDreamMoon](https://space.bilibili.com/1115826412)

- Github : [TypeDreamMoon](https://github.com/TypeDreamMoon)

- Discord : [DreamMoonDev](https://discord.gg/MsuXwGpp)

---

## 第三方资产 :

1. https://yesicon.app/

---

## 教程

|     教程     |                 YouTube                 |                                                           Bilibili                                                            |
|:----------:|:---------------------------------------:|:-----------------------------------------------------------------------------------------------------------------------------:|
|   P1 预览    | [YouTube](https://youtu.be/1s6l5LL4gdM) |   [Bilibili](https://www.bilibili.com/video/BV1rk6RYPEvc/?share_source=copy_web&vd_source=a81e6aca9f64409a68ec31aa70046f0c)   |
|   P2 基本    | [YouTube](https://youtu.be/sttFgOPDo1g) | [Bilibili](https://www.bilibili.com/video/BV1rk6RYPEvc/?p=2&share_source=copy_web&vd_source=a81e6aca9f64409a68ec31aa70046f0c) |
|  P3 歌词开发   | [YouTube](https://youtu.be/lvammFAA8II) | [Bilibili](https://www.bilibili.com/video/BV1rk6RYPEvc/?p=3&share_source=copy_web&vd_source=a81e6aca9f64409a68ec31aa70046f0c) |
| P4 音频可视化开发 | [YouTube](https://youtu.be/7b-o5JEmOmE) | [Bilibili](https://www.bilibili.com/video/BV1rk6RYPEvc/?p=4&share_source=copy_web&vd_source=a81e6aca9f64409a68ec31aa70046f0c) |

---

<p align="center">
    <img src="https://github.com/TypeDreamMoon/TypeDreamMoon_Assets/blob/main/Images/logo/TOOLKIT.png" />
</p>

> Copyright (c) 2025 - 2026 DreamMoon
[欢迎赞助](https://afdian.com/a/dreammoon)
